...
class Products : 
    ...
    async def get(self, id : int) -> types.products.Product | None  : 
        async with self.__api.get(f'products/{id}') as response : 
            if response.status == 200 : 
                return await response.json()
    
    async def products(self, params : types.products.ProductGetQuery) -> list[types.products.Product] | None : 
        async with self.__api.get(f'products?{self.__api.serialise_query_params(params, not_instance=Products)}') as response : 
            if response.status == 200 : 
                return await response.json()